      ...
      whereConditions.forEach(condition => {
        ...
        switch (type) {
          case ComparisonType.EQUAL:
            if(JsonField)
              {
                queryBuilder.andWhere(`${queryBuilder.alias}.${field}->>'${JsonField}' = :${field}`, { [field]: value });
              }
            else
              {
          ...
          case ComparisonType.LIKE:
            if(JsonField)
            {
              queryBuilder.andWhere(`${queryBuilder.alias}.${field}->>'${JsonField}' LIKE :${field}`, { [field]: `%${value}%` });
            }
          else
            {
          ...
          where.forEach(condition => {
            ...
            switch (type) {
              case ComparisonType.EQUAL:
                ...
                break;
              case ComparisonType.LIKE:
                queryBuilder.andWhere(`${relationAlias}.${field}->>'es' LIKE :${field}_rel`, { [`${field}_rel`]: `%${value}%` });
                break;
              case ComparisonType.BIGGER_THAN:
                queryBuilder.andWhere(`${relationAlias}.${field} > :${field}_rel`, { [`${field}_rel`]: value });
                break;
